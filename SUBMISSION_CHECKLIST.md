# Submission checklist (AKINDO)

**Canonical links**

- GitHub: https://github.com/AmaanSayyad/Midnight-Casino/
- Live: https://midnight-casino-eta.vercel.app/
- Deck: https://www.figma.com/deck/fIrY9l7XwfGovD0G5lSGiV/Mignight-Casino?node-id=1-1812&t=W8Z7T69GDhVJM4H9-1&scaling=min-zoom&content-scaling=fixed&page-id=0%3A1
- Demo video: https://youtu.be/DVEq_W_Uzrk

- [x] Public GitHub repository
- [x] Compiling Compact contract (`midnight-contract/casino.compact`)
- [x] Private-state / dual-ledger design documented
- [x] Simulation + tests (`npm run compact:test`)
- [x] README with setup, architecture, Midnight integration, judge path
- [x] Apache License 2.0 (`LICENSE`)
- [x] Wave progress (`WAVE1_PROGRESS.md`)
- [x] Slide deck (Figma link above + README)
- [x] Interactive demo route (`/game/privacy-wheel`)
- [x] Demo video uploaded — https://youtu.be/DVEq_W_Uzrk
- [x] Add GitHub topic/label: `midnightntwrk` (and `compact`, `typescript`, …)
- [x] Redeploy Vercel so Privacy Wheel is live on production URL
  - Team: [`amaan002s-projects`](https://vercel.com/amaan002s-projects) · Project: `midnight-casino`
  - Live: https://midnight-casino-eta.vercel.app/game/privacy-wheel
- [ ] Paste Wave progress + what changed into AKINDO submission form
