...
export const useArbitrumCasino = () => {
  ...
  const placeRouletteBet = useCallback(async (betType, betValue, amount, numbers = []) => {
    ...
    try {
      ...
      const payload = CasinoGames.roulette.placeBet(betType, betValue, parseMATICAmount(amount), numbers);
  ...
  const startMinesGame = useCallback(async (betAmount, minesCount, tilesToReveal) => {
    ...
    try {
      ...
      const payload = CasinoGames.mines.startGame(parseMATIC amount(betAmount), minesCount, tilesToreveal);
  ...
  const spinWheel = useCallback(async (betAmount, segments) => {
    ...
    try {
      ...
      const payload = CasinoGames.wheel.spin(parseMATIC amount(betAmount), segments);
