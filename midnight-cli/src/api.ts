// This file is part of APT-Casino-Midnight (casino CLI, adapted from example-counter).
...
// Pre-compile the casino contract with ZK circuit assets + private witnesses
const casinoCompiledContract = CompiledContract.make('casino', Casino.Contract).pipe(
...
export const getCasinoLedgerState = async (
  providers: CasinoProviders,
  contractAddress: ContractAddress,
) => {
  assertIsContractAddress(contractAddress);
  logger.info('Checking casino ledger state...');
  const state = await providers.publicDataProvider
    .queryContractState(contractAddress)
    .then((contractState) => (contractState != null ? Casino.ledger(contractState.data) : null));
  if (state) {
    logger.info(`Ledger: totalRounds=${state.totalRounds} nextRoundId=${state.nextRoundId}`);
  } else {
    logger.info('No casino contract state found');
  }
  return state;
};

export const casinoContractInstance: CasinoContract = new Casino.Contract(createWitnesses() as any);

export const joinContract = async (
  providers: CasinoProviders,
  contractAddress: string,
  secretKey: Uint8Array,
): Promise<DeployedCasinoContract> => {
  const casinoContract = await findDeployedContract(providers, {
    contractAddress,
    compiledContract: casinoCompiledContract,
    privateStateId: CasinoPrivateStateId,
    initialPrivateState: createCasinoPrivateState(secretKey),
  });
  logger.info(`Joined contract at address: ${casinoContract.deployTxData.public.contractAddress}`);
...
export const deploy = async (
  ...
  logger.info('Deploying casino contract...');
  const casinoContract = await deployContract(providers, {
    compiledContract: casinoCompiledContract,
    privateStateId: CasinoPrivateStateId,
    initialPrivateState: createCasinoPrivateState(secretKey),
  });
  logger.info(`Deployed contract at address: ${casinoContract.deployTxData.public.contractAddress}`);
  return casinoContract;
};

export const placeBet = async (
  providers: CasinoProviders,
  casinoContract: DeployedCasinoContract,
  gameType: number,
  choice: number,
  amount: bigint,
): Promise<FinalizedTxData> => {
  const existing = await providers.privateStateProvider.get(CasinoPrivateStateId);
  if (!existing) throw new Error('Missing casino private state');
  const salt = crypto.getRandomValues(new Uint8Array(32));
  await providers.privateStateProvider.set(CasinoPrivateStateId, setBetInputs(existing, choice, amount, salt));
  logger.info(`Placing private bet gameType=${gameType} choice=${choice}`);
  const finalizedTxData = await casinoContract.callTx.placeBet(BigInt(gameType));
  logger.info(`placeBet tx ${finalizedTxData.public.txId} in block ${finalizedTxData.public.blockHeight}`);
  return finalizedTxData.public;
};

export const commitHouseSeed = async (
  casinoContract: DeployedCasinoContract,
  commit: Uint8Array,
): Promise<FinalizedTxData> => {
  logger.info('Committing house seed...');
  const finalizedTxData = await casinoContract.callTx.commitHouseSeed(commit);
  logger.info(`commitHouseSeed tx ${finalizedTxData.public.txId} in block ${finalizedTxData.public.blockHeight}`);
  return finalizedTxData.public;
};

export const settleWheel = async (
  casinoContract: DeployedCasinoContract,
  roundId: bigint,
  houseOutcome: number,
): Promise<FinalizedTxData> => {
  logger.info(`Settling round ${roundId} with houseOutcome=${houseOutcome}`);
  const finalizedTxData = await casinoContract.callTx.settleWheel(roundId, BigInt(houseOutcome));
  logger.info(`settleWheel tx ${finalizedTxData.public.txId} in block ${finalizedTxData.public.blockHeight}`);
  return finalizedTxData.public;
};
...
${DIV}
  ...
  └─ Address: ${
    state.dust?.address
      ? MidnightBech32m.encode(networkId, state.dust.address).toString()
      : '(syncing)'
  }
...
export const configureProviders = async (ctx: WalletContext, config: Config) => {
  ...
  const zkConfigProvider = new NodeZkConfigProvider<CasinoCircuits>(contractConfig.zkConfigPath);
  ...
  return {
    privateStateProvider: levelPrivateStateProvider<typeof CasinoPrivateStateId>({
